...
public static class SudokuHandler {
    ...
    private static bool GenerateSudokuHelper(int[,] board) {
        generationCount += 1;
        //Console.WriteLine("Generation recursive called {0} times.", generationCount);
        int row, col;
    ...
    public static SudokuSequence GetHint(int[,] board) {
        SudokuSequence hint = new SudokuSequence();
        int row = 0, col = 0;
        bool isDone = true;
        ...
        for (row = 0; row < GridSize; row++) {
            for (col = 0; col < GridSize; col++) {
                if (board[row, col] == 0) {
                    for (int num = 1; num <= GridSize; num++) {
                        if (IsSafe(board, row, col, num)) {
                            hint.positionRow = row;
                            hint.positionCol = col;
                            hint.numberBefore = board[row, col];
                            hint.numberAfter = num;
                            return hint;
                        }
                    }
                }
